...
> R:No, no coincide. Debería devolver 2800 pero devuelve 2050.
...
> R:carrito_init es el que se encarga de inicializar las variables relacionadas al carrito. Si no se pone antes de las otras funciones que usan el carro, el código no compilará pues no reconoce las variables llamadas al no estar inicializadas.
...
> R:Este test falla, esperaba 700 pero recibe 350
...
> R:El bug está en la línea 24, falta multiplicar los productos por su cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Habría que añadir una multiplicación entre el precio y la cantidad
...
> R:Si, el test falla. Devuelve 1 en lugar de 0.
...
> R:No se ejecutan las líneas 29 y 30, pues ningún test las llaman.
...
> R:Un test unitario prueba funciones aisladas. Un test de integracion verifica el correcto funcionamiento de las funciones en conjunto.
...
> R: En C no se verifica el limite de un array, mientras que en Python y Java si se verifica, pudiendo lanzar una excepcion cuando se supera dicho límite.
...
> R:Leer el código puede ser engañoso, pues si se ve muy coherente uno puede llegar a pasar por alto un error. Tener estos tests nos permite ver que realiza este código de forma adecuada
...
> R: No se detectaba el error porque la cantidad es igual al precio unitario de un objeto, entonces la cantidad calculada coincide con el código bugeado.
